...
export default function PersistentDrawerLeft() {
  ...
  React.useEffect(() => {
    axios
      .get(
        `https://loc-backend-acm.herokuapp.com/student/search_batches?userId=${localStorage.getItem(
          "userId"
        )}`
      )
      .then((response) => {
...
React.useEffect(() => {
  ...
  axios.get(`https://loc-backend-acm.herokuapp.com/student/enroll_batch/?userId=${localStorage.getItem("userId")}&batchId=${}`
...
*/
  React.useEffect(() => {
    axios
      .get(
        `https://loc-backend-acm.herokuapp.com/student/enrolled_batches/?studentId=${localStorage.getItem(
          "userId"
        )}`
      )
      .then((response) => {
  ...
  const handleSendId = (event) => {
    ...
    axios
      .get(
        `https://loc-backend-acm.herokuapp.com/student/enroll_batch/?studentId=${localStorage.getItem(
          "userId"
        )}&batchId=${uid}`
      )
      .then((response) => {
    ...
    <div>
            ...
            <Grid item xs={4}>
              ...
              >
                <div
                  style={{
                    height: "60%",
                    backgroundColor: color_arr[item.info.subject.length % 10],
                    borderRadius: "10px",
                    paddingLeft: "10px",
                    paddingRight: "10px",
                  }}
                  ...
                  <Grid container spacing={2}>
                    <Grid item xs={8}>
                      <h1 style={{ color: "white" }}>
                        {item.info.subject.toUpperCase()}
                      </h1>
                    </Grid>
                    <Grid item xs={4}>
                      <img
                        src={image_arr[item.info.subject.length % 5]}
      ...
      >
        <DialogTitle id="alert-dialog-title">
          {"Are you sure you want to enroll in this batch?"}
        </DialogTitle>
        <DialogContent>
